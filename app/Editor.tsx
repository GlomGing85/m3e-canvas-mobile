...
export default function Editor({ initialLang, onReady }: { initialLang: Lang; onReady?: () => void }) {
  ...
  /* PATCHED: desktop mode forced on all devices - no phone limitation */
  useEffect(() => {
    // Check for explicit override: ?forceMobile=1 or localStorage m3e:forceMobile
    const params = typeof window !== "undefined" ? new URLSearchParams(window.location.search) : null;
    const forceMobileParam = params?.get("forceMobile") === "1" || params?.get("mobile") === "1";
    const forceDesktopParam = params?.get("forceDesktop") === "1" || params?.get("desktop") === "1";
    const storedForce = typeof window !== "undefined" ? localStorage.getItem("m3e:forceMobile") : null;
    
    let m = false;
    if (forceMobileParam) m = true;
    else if (forceDesktopParam) m = false;
    else if (storedForce === "1") m = true;
    else if (storedForce === "0") m = false;
    else {
      // Default: ALWAYS desktop - the restriction is removed
      m = false;
    }
    ...
    const apply = (isMobileDevice: boolean) => {
      // If user explicitly forced mobile, respect it, otherwise stay desktop
      const shouldBeMobile = forceMobileParam ? isMobileDevice : m;
      setIsMobile(shouldBeMobile);
      mobileRef.current = shouldBeMobile;
      if (shouldBeMobile) {
        setMode("select");
        setSheet(null);
    ...
    apply(mq.matches);
    // Still listen for resize but keep desktop unless forceMobile is set
    const listener = () => {
      if (forceMobileParam) apply(mq.matches);
    };
  ...
  const toggleMobileMode = useCallback(() => {
    const next = !isMobile;
    try {
      localStorage.setItem("m3e:forceMobile", next ? "1" : "0");
    } catch {}
    setIsMobile(next);
    ...
    queueMicrotask(() => fitRef.current());
    showToast(next ? "📱 Mobile mode" : "💻 Desktop unlocked - full editor!", 2200, next ? "smartphone" : "computer");
  }, [isMobile, lang]);
  ...
  const addKind = (kind: Kind) => {
    ...
    const item = makeItem(kind);
    const sz = sizeOf(item, widthsRef.current);
    const f = framesRef.current.find(ff => ff.id === selectedFrameId) ?? framesRef.current[0];
    let x = ((r?.width ?? 0) / 2 - v.x) / v.z - sz.w / 2;
    let y = ((r?.height ?? 0) / 2 - v.y) / v.z - sz.h / 2;
    if (f) {
      const { w, h } = frameSizeOf(f);
        ...
        >
          ...
          {isMobile && (
            <MobileBottomNav
              p={p}
              active={sheet}
              onChange={(s) => setSheet(s === sheet ? null : s)}
              onAdd={() => setSheet(sheet === "parts" ? null : "parts")}
            />
          )}
          ...
          <AnimatePresence>
            ...
            {isMobile && sheet === "parts" && (
              <BottomSheet key="parts" p={p} onClose={() => setSheet(null)} maxHeight="88%">
                <MobilePartsSheet p={p} onAdd={(k) => { addKind(k); setSheet(null); }} onClose={() => setSheet(null)} />
              </BottomSheet>
            )}
            {isMobile && sheet === "layers" && (
              <BottomSheet key="layers" p={p} onClose={() => setSheet(null)} maxHeight="82%">
                <MobileLayersSheet p={p} onClose={() => setSheet(null)}>
                  <LayersPanel
                    p={p}
                    frames={frames}
                    frameId={layersFrame?.id ?? null}
            ...
            {isMobile && sheet === "theme" && (
              <BottomSheet key="theme" p={p} onClose={() => setSheet(null)} maxHeight="88%">
                <MobileThemeSheet p={p} paletteKey={paletteKey} onPalette={setPaletteKey} theme={theme} onTheme={patchTheme} onClose={() => setSheet(null)} />
              </BottomSheet>
            )}
